...
class HardFilterThresholds:
    ...
    def as_expression(self, sample: Sample | None = None) -> str:
        ...
        sample = sample or "0"
        ret = (
            f"FMT/DP[{sample}]>={self.min_dp} && "
            f"FMT/AD[{sample}:1]>={self.min_ad} && "
            f"(FMT/AD[{sample}:1]/FMT/DP[{sample}])>={self.min_vaf}"
        )
        return ret
