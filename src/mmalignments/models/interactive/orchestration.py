...
class CompositeRenderer:
    ...
    def render(
        self, result: AnalysisResult, combined_state: Parameterized
    ) -> "go.Figure":
        fig = None
        for layer in self.layers:
            ...
            fig = plot.render(df, layer, layer_state, self.processor, fig=fig)
...
class SelectiveRenderer:
    def __init__(
        ...
        """
        self.layers_by_alias: dict[str, PlotLayer] = {l.alias: l for l in layers}
    ...
    def render(
        ...
        fig = plot.render(df, layer, layer_state, self.processor, fig=None)
        if fig is not None:
