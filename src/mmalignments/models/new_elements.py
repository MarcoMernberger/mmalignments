...
from .specs import Runnable, CallSpec, ValidationPolicy  # type: ignore[import]
...
class Element(Prerequisite):
    ...
    def output_files(self) -> Iterable[Path] | None:
        ...
        for k, v in self.artifacts.items():
            if hasattr(v, "resolve"):  # covers Path, TableArtifact, FileArtifact
                files.append(v.resolve())
            elif hasattr(v, "files"):
    ...
    def __init__(
        ...
        *,
        ...
        if self._producer is not None:
            ...
            print("SETM", self._artifacts.primary)
            tag = ElementTag(
                root=self._artifacts.primary.stem,
                level=0,
                omics=Omics.DNA,
