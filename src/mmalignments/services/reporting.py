    ...
    def _generate_summary_stats(
        ...
        def generate():
            with open(output_file, "w") as f:
                ...
                if mapping_rates:
                    f.write(
                        f"  Mean mapping rate: {sum(mapping_rates) / len(mapping_rates):.1f}%\n"
                    )
                if duplication_rates:
                    f.write(
                        f"  Mean duplication rate: {sum(duplication_rates) / len(duplication_rates):.1f}%\n"
                    )
                if mean_coverages:
                    f.write(
                        f"  Mean coverage: {sum(mean_coverages) / len(mean_coverages):.1f}x\n"
                    )
        ...
        def plot():
            try:
                ...
                with open(output_file.with_suffix(".txt"), "w") as f:
                    ...
                    for ml in mutational_loads:
                        f.write(
                            f"{ml.sample_name}: {ml.mutational_load:.2f} mutations/Mb\n"
                        )
