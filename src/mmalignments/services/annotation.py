    ...
    def parse_vcf_variants(
        ...
        with open_func(vcf_file, "rt") as f:
            ...
            for line in f:
                ...
                sample_data = (
                    parts[9 + sample_idx].split(":")
                    if len(parts) > 9 + sample_idx
                    else []
                )
                ...
                if "DP" in format_fields and len(sample_data) > format_fields.index(
                    "DP"
                ):
                    try:
                ...
                if "AD" in format_fields and len(sample_data) > format_fields.index(
                    "AD"
                ):
                    try:
